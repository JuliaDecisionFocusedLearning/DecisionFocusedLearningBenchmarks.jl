...
function plot_scenario(
    ...
    edge_colors = [
        ...
        elseif e2
            ...
            :black
        end for (e1, e2) in zip(first_stage_forest, second_stage_forests[:, scenario])
    ]
    edge_widths = [
        e1 || e2 ? 3 : 1 for
        (e1, e2) in zip(first_stage_forest, second_stage_forests[:, scenario])
    ]
    weights =
        first_stage_forest .* first_stage_costs +
        .!first_stage_forest .* second_stage_costs[:, scenario]
