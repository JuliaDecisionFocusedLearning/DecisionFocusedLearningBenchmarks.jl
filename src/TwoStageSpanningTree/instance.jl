...
function Base.show(io::IO, instance::TwoStageSpanningTreeInstance)
    return print(
        io,
        "$(collect(edges(instance.graph))), $(instance.first_stage_costs), $(instance.second_stage_costs)",
    )
end

function TwoStageSpanningTreeInstance(;
    n, m, nb_scenarios=1, c_range=1:20, d_range=1:20, seed=nothing, type=Float64
)
    g = Graphs.grid((n, m))
