...
"""
function MILP_separation_problem(
    graph, weights; model_builder=() -> Model(HiGHS.Optimizer), tol=1e-6
)
    V = nv(graph)
    E = ne(graph)
    ...
    @objective(model, Min, sum(α[v] for v in 1:V) - 1 - sum(weights[e] * β[e] for e in 1:E))

    @constraint(
        model,
        [(e, edge) in enumerate(edges(graph))],
        2 * β[e] <= α[dst(edge)] + α[src(edge)]
    )
    ...
    optimize!(model)
    found = JuMP.objective_value(model) + tol <= 0
...
function build_flow_graph(graph, weights; infinity=1e6)
    ...
    sources = vcat(
        fill(o, E), [2 + e for e in 1:E], [2 + e for e in 1:E], [2 + E + v for v in 1:V]
    )
    destinations = vcat(
        [2 + e for e in 1:E],
    ...
    costs = vcat([weights[e] for e in 1:E], fill(infinity, 2 * E), ones(V))
...
"""
function cut_separation_problem(
    graph, weights; model_builder=() -> Model(HiGHS.Optimizer), tol=1e-6
)
    ...
    @objective(model, Min, sum(costs[a] * β[a] for a in 1:A))

    @constraint(model, α[1] == 1)
    @constraint(model, α[2] == 0)

    @constraint(model, [a in 1:A], β[a] >= α[sources[a]] - α[destinations[a]])

    @constraint(model, sum(α[v] for v in vertex_range) >= 1)

    optimize!(model)

    min_cut_value = JuMP.objective_value(model)
...
function cut_generation(
    ...
    separation_problem=MILP_separation_problem,
    ...
    @expression(model, first_stage_objective, sum(first_stage_costs[e] * y[e] for e in 1:E))
    @expression(
        model,
    ...
    # Add constraints
    @constraint(model, [s in 1:S], sum(y[e] + z[e, s] for e in 1:E) == V - 1)
    ...
    function my_callback_function(cb_data)
        ...
        for current_s in 1:S
            ...
            found, Y, YY = separation_problem(graph, weights; model_builder=model_builder)
