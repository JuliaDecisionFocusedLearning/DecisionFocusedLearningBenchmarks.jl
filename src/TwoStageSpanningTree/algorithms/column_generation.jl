...
function column_generation(
    instance; model_builder=() -> Model(GLPK.Optimizer), tol=1e-6, verbose=true
)
    ...
    @constraint(model, [e in 1:E, s in 1:S], second_stage_costs[e, s] / S - μ[e, s] >= 0)

    @constraint(model, [e in 1:E], first_stage_costs[e] - sum(μ[e, s] for s in 1:S) >= 0)

    trees = BitVector[]

    for s in 1:S
        _, dummy_tree = kruskal(graph, min.(first_stage_costs, second_stage_costs[:, s]))
        @constraint(model, ν[s] <= sum(μ[e, s] for e in 1:E if dummy_tree[e]))
    ...
    return (; value=JuMP.objective_value(model), ν=value.(ν), μ=value.(μ), columns=trees)
end

function column_heuristic(instance, columns; model_builder=() -> Model(HiGHS.Optimizer))
    (; graph, first_stage_costs, second_stage_costs) = instance
    E = ne(graph)
    S = nb_scenarios(instance)
    T = length(columns)
...
"""
function column_heuristic(instance; model_builder=() -> Model(GLPK.Optimizer), verbose=true)
    (; columns) = column_generation(instance; verbose=false, model_builder)
